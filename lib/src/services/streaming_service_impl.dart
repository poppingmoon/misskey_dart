...
class StreamingService implements StreamingController, WebSocketController {
  ...
  Stream<StreamingResponse> addChannel(
    ...
    log("addChannel activeStreams: $_activeStreams / ${streamingUrl.host}");
  ...
  Future<void> removeChannel(String id) async {
    try {
      ...
      log(
        "removeChannel activeStreams: $_activeStreams / ${streamingUrl.host}",
      );
      sendRequest(
      ...
      _timer = Timer(Duration(seconds: 5), () async {
        log("activeStreams: $_activeStreams / ${streamingUrl.host}");
        if (_activeStreams == 0) {
          _subNotes.clear();
